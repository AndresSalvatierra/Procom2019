...
def main():
    ...
    canal=np.convolve(rc_fp,rch_fp)  ## filtro equivalente, resultado de la convolucion de filtro Tx y FIR canal
    ...
    out_tx_I=0
    out_tx_Q=0
    out_tx_I_fp= DeFixedInt(8,6,'S','trunc') #Punto Fijo
    out_tx_Q_fp= DeFixedInt(8,6,'S','trunc')

    #Salida de del canal mas el ruido
    out_ch_I=0
    out_ch_Q=0

    out_ch_I_fp= DeFixedInt(9,7) #Punto Fijo
    out_ch_Q_fp= DeFixedInt(9,7) 

    #Salida del filtro receptor
    rx_I = 0
    rx_Q = 0 
    rx_I_fp = DeFixedInt(9,7,'S','trunc') #Punto Fijo
    rx_Q_fp = DeFixedInt(9,7,'S','trunc')
    ...
    y_I=0
    y_Q=0
    y_I_fp=DeFixedInt(11,9,'S','trunc')
    y_Q_fp=DeFixedInt(11,9,'S','trunc')
    
    #Error realimentacion Equalizador
    error_adap_I = 0
    error_adap_Q = 0
    error_adap_I_fp = DeFixedInt(9,7,'S','trunc')
    error_adap_Q_fp = DeFixedInt(9,7,'S','trunc')
    ...
    coef_fir_adap_I_fp=DeFixedInt(15,13,'S','trunc')
    coef_fir_adap_Q_fp=DeFixedInt(15,13,'S','trunc')

    noise_vector_I_fp = DeFixedInt(9,7,'S','trunc')
    noise_vector_Q_fp = DeFixedInt(9,7,'S','trunc')

    sym_t_I_fp=DeFixedInt(9,7,'S','trunc')
    sym_t_Q_fp=DeFixedInt(9,7,'S','trunc')
    ...
    for t in range(len(snr_iteraciones)):
        error_final=0
        noise_vector_I=noise(snr[t],snr_iteraciones[t],energia_fir_equiv/4)#/(os**2)) #genero senial de ruido
        noise_vector_Q=noise(snr[t],snr_iteraciones[t],energia_fir_equiv/4)#/(os**2))
        ...
        for i in range(0,snr_iteraciones[t]):
            ...
            mem_canal_I[0]=out_tx_Q_fp.fValue #agregamos nuevo valor de salida de Tx a la memoria

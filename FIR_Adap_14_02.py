...
def main():
    ...
    mem_filtro_I= np.zeros(Ntap) #Memoria del filtro FIR
    mem_filtro_Q= np.zeros(Ntap) #Memoria del filtro FIR
    coef_I=np.zeros(Ntap) #Coeficientes del filtro FIR del equalizador adaptativo
    coef_Q=np.zeros(Ntap) #Coeficientes del filtro FIR del equalizador adaptativo
    ...
    for i in range(0,iteraciones):
        ...
        if(((i+1)%os)==0):
           ...
           # print "FIR", y_I
            ...
            # for b in range(0,Ntap-1):
            #     coef_I[b]=coef_I[b] + delta*error_adap_I*mem_filtro_I[b]
            #     coef_Q[b]=coef_Q[b] + delta*error_adap_Q*mem_filtro_Q[b]

            if(graficar==1):
                senial_recibida_I.append(y_I)  #Usada para graficar
                senial_recibida_Q.append(y_Q)  #Usada para graficar
                # grafica_error_I.append((error_adap_I**2)/len(grafica_error_I))
                # grafica_error_Q.append((error_adap_Q**2)/len(grafica_error_I))
    ...
    ##ERROR ADAPTATIVO
    plt.figure()
    plt.semilogy(grafica_error_I) 
    plt.semilogy(grafica_error_Q)

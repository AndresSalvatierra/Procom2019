...
def cuantizacion (rc,NB,NBF,ajuste):
    data= []
    rc_aux=arrayFixedInt(NB,NBF,rc,signedMode='S',roundMode=ajuste,saturateMode='saturate')
    
    for i in range(len(rc_aux)):
        data.append(rc_aux[i].fValue)
    return data

def main():
    
    # filep = open('prbsi.txt','w') #Genero el archivo(PRBSI)
    # filep.close()
    # filep = open('txi.txt','w') #Genero el archivo(txi)
    # filep.close()
    # filep = open('rxi.txt','w') #Genero el archivo(rxi)
    # filep.close()
    # filep = open('prbsq.txt','w') #Genero el archivo(PRBSQ)
    # filep.close()
    ...
    rc0_t=cuantizacion(rc,8,7,'trunc')
    ...
    plt.plot(rct,'k-',linewidth=2.0,label=r'$\beta=0.5$')
    plt.legend()
    plt.grid(True)
    plt.xlim(0,len(rct)-1)
    plt.xlabel('Muestras')
    plt.ylabel('Magnitud')

    ## Respuesta en frecuencia
    [H0,A0,F0]=resp_freq(rct, Ts, Nfreqs)  
    ...
    b_I=DeFixedInt(9,7,'S','trunc')  #Salida de convolucion transmisor
    b_Q=DeFixedInt(9,7,'S','trunc')  #Salida de convolucion transmisor
    rx_pfi_I=DeFixedInt(16,14,'S','trunc')  #Salida de convolucion receptor
    rx_pfi_Q=DeFixedInt(16,14,'S','trunc')  #Salida de convolucion receptor
    ...
    for i in range(0,iteraciones):
        ...
        tx_I=np.sum(shift_tx_I*rct)
        tx_Q=np.sum(shift_tx_Q*rct)
        
        b_I.value=tx_I
        # if(graficar==1):
        #     filep = open('txi.txt','a')
        #     filep.write(str(b_I.intvalue))
        #     filep.write("\n")
        #     filep.close()
        b_Q.value=tx_Q
        ...
        rx_I=np.sum(shift_rx_I*rct)
        rx_Q=np.sum(shift_rx_Q*rct)

        rx_pfi_I.value=rx_I
        # if(graficar==1):
        #     filep = open('rxi.txt','a')
        #     filep.write(str(rx_pfi_I.intvalue))
        #     filep.write("\n")
        #     filep.close()
        rx_pfi_Q.value=rx_Q
        
        if(graficar==1):
            senial_recibida_I.append(rx_pfi_I.fValue)  #Usada para graficar
            senial_recibida_Q.append(rx_pfi_Q.fValue)  #Usada para graficar

        for l in range(0,len(phase_I)-1):
            phase_I[l]=phase_I[l+1]
            phase_Q[l]=phase_Q[l+1]

        #Slicer
        if(rx_pfi_I.fValue>=0):
            rx_I=1
        else:
            rx_I=-1
        
        if(rx_pfi_Q.fValue>=0):
            rx_Q=1
        else:
            rx_Q=-1

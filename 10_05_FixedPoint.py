...
def main():
    ...
    rx_I = 0
    rx_Q = 0 
    rx_I_fp = DeFixedInt(8,7,'S','trunc') #Punto Fijo
    rx_Q_fp = DeFixedInt(8,7,'S','trunc')
    ...
    for t in range(len(snr_iteraciones)):
        error_final=0
        noise_vector_I=noise(snr[t],snr_iteraciones[t],0) #genero senial de ruido
        ...
        for i in range(0,snr_iteraciones[t]):
            ...
            if(i==0):
                ...
                sym_t_I=out_ch_I_fp.fValue+noise_vector_I[i]
                sym_t_Q=out_ch_Q_fp.fValue+noise_vector_Q[i]
    ...
    print "Promedio Error", np.mean(promedio_error)
    print "Promedio Error FixedP", np.mean(promedio_error2)
